...
void copyin(void* to, uint64_t from, size_t size) {
    ...
    mach_vm_read_overwrite(tfp0, from, size, (mach_vm_offset_t)to, &outsize);
}
...
\
    ...
    NSLog(@"level %d at %llx - %llx", addr, tte_addr, TTE_GET(tte, TTE_PHYS_VALUE_MASK));\
...
TTE_SET(tte, TTE_BLOCK_ATTR_PXN_MASK, 0);\
    NSLog(@"got %llx %llx", tte_addr, findphys_real(newt));\
    sleep(1);\
        WriteAnywhere64(tte_addr, tte);\
